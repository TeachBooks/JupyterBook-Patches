...
- with a `mathjax` patch:
    ...
    - that make the default loading method of MathJax to be lazy loading, which improves page load times. Can be disabled if needed by either
      - disabling the `mathjax` patch;
      - adding `ui/<value>` to `config.mathjax3_config['loader']['load']` in your config file, where `<value>` can be any of the next values:
        - `menu`
        - `assistive-mml`
        - `safe`
        - `nonlazy` (_This value is not defined by MathJax, but introduced by JupyterBook-Patches to allow disabling lazy loading._)
